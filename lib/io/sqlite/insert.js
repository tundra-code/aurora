import { Attributes, noteFromNoteModel, attrFromAttrModel } from "./models.js";
...
function saveAttributes(note, savedNote, callback, onFailure) {
  ...
  loadDB()
    .then(() => {
      ...
      bookshelfNote
        .save()
        .then(savedNote => {
          saveAttributes(note, savedNote, callback, onFailure);
        })
        .catch(err => {
          executeIfDefined(onFailure, err);
        });
